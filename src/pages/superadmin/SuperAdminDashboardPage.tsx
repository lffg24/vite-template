...
export default function SuperAdminDashboardPage() {
  ...
  const k = data?.kpis || {};
  return <div><SuperAdminPageHeader title="Dashboard Super Usuario" subtitle="Resumen general de la plataforma ABRIL360." action={<button className="rounded-2xl border bg-white px-5 py-3 font-black text-brand-primary">Exportar reporte</button>} />{error && <div className="mb-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-700">{error}</div>}<section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4"><SuperAdminCard title="Empresas registradas" value={k.empresas ?? "—"} hint="Total plataforma" icon={Building2} /><SuperAdminCard title="Usuarios registrados" value={k.usuarios ?? "—"} hint="Incluye psicólogos y admins" icon={UsersRound} /><SuperAdminCard title="Aplicaciones totales" value={k.aplicaciones ?? "—"} hint="Baterías psicosociales" icon={ClipboardList} /><SuperAdminCard title="Créditos disponibles" value={k.saldo_total ?? "—"} hint={`Asignados: ${k.asignados_total ?? 0}`} icon={CreditCard} /></section><section className="mt-6 grid gap-5 xl:grid-cols-2"><div className="rounded-3xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Consumo de créditos</h2><div className="mt-6 grid place-items-center rounded-3xl bg-slate-50 p-10 text-center"><div className="text-5xl font-black text-brand-primary">{k.consumidos_total ?? 0}</div><div className="mt-2 font-bold text-slate-500">Créditos consumidos</div></div></div><div className="rounded-3xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Alertas y notificaciones</h2><div className="mt-5 grid gap-3"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 font-bold text-amber-700">Revisar cuentas con saldo bajo</div><div className="rounded-2xl border border-accent bg-accent p-4 font-bold text-brand-primary">Auditoría activa para acciones SuperAdmin</div></div></div></section></div>;
}
