...
export default function EmpresaEmpleadosPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 pb-6 pt-2 lg:px-8 lg:pb-8 lg:pt-3">
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-black text-slate-950">Empleados</h1><p className="text-sm text-slate-500">Listado de colaboradores vinculados a esta empresa.</p></div><div className="flex flex-wrap gap-3"><button onClick={() => setOpenManual(true)} className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:bg-primary-hover"><Plus className="h-4 w-4" /> Agregar manualmente</button><button onClick={() => setOpenBulk(true)} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-800 hover:bg-slate-50"><Upload className="h-4 w-4" /> Carga masiva</button></div></div></section>
...
export function BulkUploadModal({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
    <section className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-[24px] border border-white/60 bg-white shadow-[0_30px_90px_rgba(15,23,42,0.35)]">
      <div className="relative shrink-0 overflow-hidden border-b border-slate-200 bg-gradient-to-r from-white via-white to-brand-canvas px-5 py-4 sm:px-6">
        <div className="absolute inset-y-0 right-0 w-1/3 bg-accent/70 [clip-path:ellipse(80%_70%_at_80%_50%)]" />
        <div className="relative flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-card">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        ...
        <label className={`flex cursor-pointer flex-col items-center justify-center rounded-[22px] border-2 border-dashed border-accent bg-gradient-to-br from-accent via-white to-brand-canvas px-5 text-center transition hover:border-input-focus hover:from-accent/80 ${file ? "min-h-32 py-4" : "min-h-44 py-6"}`}>
          <div className="relative">
            <FileSpreadsheet className={`${file ? "h-12 w-12" : "h-16 w-16"} text-emerald-600`} />
            <span className="absolute -right-3 bottom-0 flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-card"><Upload className="h-4 w-4" /></span>
          </div>
        ...
        {(fileReady || saving || result) && <div className="mt-4 rounded-[20px] border border-slate-200 bg-white p-3 shadow-sm">
          {result ? <div className="grid gap-3 md:grid-cols-3 md:items-center">
            ...
            {saving ? <Loader2 className="h-5 w-5 animate-spin text-brand-primary" /> : <FileSpreadsheet className="h-5 w-5 text-brand-primary" />}
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1.2fr]">
          <div className="rounded-[20px] border border-accent bg-accent/70 p-4 sm:flex sm:items-center sm:justify-between lg:block">
            <div className="flex items-center gap-4">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-brand-primary ring-1 ring-accent"><FileDown className="h-5 w-5" /></span>
              <div>
                <p className="text-sm font-black text-slate-950">¿No tienes el archivo?</p>
                <p className="mt-1 text-xs font-semibold text-slate-600">Usa el formato estándar.</p>
              </div>
            </div>
            <a href="/templates/plantilla_carga_colaboradores.xlsx" download className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-accent bg-white px-4 py-2.5 text-sm font-black text-brand-primary hover:bg-accent sm:mt-0 sm:w-auto lg:mt-3 lg:w-full"><Download className="h-4 w-4" /> Descargar formato</a>
          </div>
      ...
      <div className="sticky bottom-0 z-10 flex shrink-0 flex-col gap-3 border-t border-slate-200 bg-white/95 px-5 py-3 backdrop-blur sm:flex-row sm:justify-end sm:px-6">
        ...
        <button disabled={!file || saving} onClick={onValidate} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-accent px-5 py-2.5 text-sm font-black text-brand-primary hover:bg-accent disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <ShieldCheck className="h-5 w-5" />} Validar archivo</button>
        <button disabled={!canImport || saving} onClick={onImport} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-2.5 text-sm font-black text-primary-foreground hover:bg-primary-hover disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <CloudUpload className="h-5 w-5" />} Importar empleados</button>
      </div>
    </section>
  </div>;
}
...
function StepPill({ active, number, title, subtitle }: { active: boolean; number: number; title: string; subtitle: string }) {
  return <div className="flex items-center gap-3">
    <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-black ${active ? "bg-primary text-primary-foreground shadow-card" : "border border-slate-300 bg-slate-50 text-slate-600"}`}>{number}</span>
...
function MetricInline({ icon, label, value, tone }: { icon: ReactNode; label: string; value: number; tone: "violet" | "emerald" | "amber" }) {
  const toneClass = {
    violet: "bg-accent text-brand-primary",
    ...
    <div className="flex flex-col gap-3 border-b border-slate-100 bg-gradient-to-r from-white to-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 outline-none transition focus:border-input-focus focus:ring-4 focus:ring-input-focus/20"
        >
    ...
    <div className="overflow-x-auto">
      <table className="min-w-[860px] w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          ...
          {items.map((e) => <tr key={e.id} className="transition hover:bg-accent/40">
            <td className="px-4 py-4">
              <button onClick={() => navigate(`/psicosocial/empleados/${e.id}`)} className="group inline-flex items-center gap-3 text-left">
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-accent text-brand-primary ring-1 ring-accent transition group-hover:bg-primary group-hover:text-primary-foreground"><UserRound className="h-5 w-5" /></span>
                <span className="min-w-0">
                  <strong className="block max-w-[280px] truncate text-slate-900 group-hover:text-brand-primary group-hover:underline">{e.nombre}</strong>
...
function Input({ value, onChange, className = "", ...props }: any) { return <input {...props} value={value} onChange={(e) => onChange(e.target.value)} className={`w-full rounded-2xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-input-focus focus:ring-4 focus:ring-input-focus/20 ${className}`} />; }
function Combo({ value, label, options, onChange, disabled, placeholder, emptyMessage, helper }: { value: string; label: string; options: Array<{ value: string; label: string }>; onChange: (v: string) => void; disabled?: boolean; placeholder?: string; emptyMessage?: string; helper?: string }) {
  ...
  return <div ref={rootRef} className="relative flex-1"><button type="button" disabled={disabled} onClick={() => setOpen((v) => !v)} className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left font-normal outline-none transition focus:border-input-focus focus:ring-4 focus:ring-input-focus/20 ${disabled ? "cursor-not-allowed border-slate-200 bg-slate-50 text-slate-400" : open ? "border-brand-primary/30 bg-accent/60" : "border-slate-200 bg-white hover:border-brand-primary/30"}`}><div className="min-w-0"><span className={`block truncate font-semibold ${value ? "text-slate-800" : "text-slate-400"}`}>{disabled ? (helper || "Selecciona primero un área") : label}</span>{helper && !disabled && <span className="mt-0.5 block text-xs text-slate-400">{helper}</span>}</div><ChevronDown className={`h-4 w-4 shrink-0 text-slate-400 transition ${open ? "rotate-180 text-brand-primary" : ""}`} /></button>{open && !disabled && <div className="absolute z-[70] mt-2 w-full overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.16)]"><div className="border-b border-slate-100 bg-slate-50 px-4 py-3"><p className="text-xs font-black uppercase tracking-[0.16em] text-brand-primary">Catálogo</p><p className="mt-1 text-sm font-semibold text-slate-600">{placeholder || "Selecciona una opción"}</p></div><div className="max-h-64 overflow-auto p-2">{options.length > 0 ? <><button type="button" onClick={() => { onChange(""); setOpen(false); }} className="mb-1 flex w-full items-center justify-between rounded-2xl px-3 py-2.5 text-left text-sm font-semibold text-slate-500 transition hover:bg-slate-50">Limpiar</button>{options.map((opt) => <button key={opt.value} type="button" onClick={() => { onChange(opt.value); setOpen(false); }} className={`flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left text-sm font-bold transition ${value === opt.value ? "bg-accent text-brand-primary ring-1 ring-accent" : "text-slate-700 hover:bg-slate-50"}`}><div className="min-w-0"><span className="block truncate">{opt.label}</span>{value === opt.value && <span className="mt-0.5 block text-xs font-semibold text-brand-primary/80">Seleccionado</span>}</div>{value === opt.value && <Check className="h-4 w-4" />}</button>)}</> : <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-5 text-center text-sm text-slate-500">{emptyMessage || "No hay opciones disponibles"}</div>}</div></div>}</div>; }
