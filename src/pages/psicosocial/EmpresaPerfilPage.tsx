      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="grid gap-5 lg:grid-cols-[1fr_420px]">
            <div className="flex items-start gap-5"><div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-violet-100 text-violet-700"><Building2 className="h-10 w-10" /></div><div><h1 className="text-3xl font-black text-slate-950">{empresa.nombre}</h1><p className="mt-1 text-sm text-slate-500">NIT {empresa.nit || "Sin dato"} · {empresa.ciudad || "Sin ciudad"} · {empresa.email || "Sin correo"}</p><span className="mt-3 inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">{empresa.estado || "Activa"}</span></div></div>
