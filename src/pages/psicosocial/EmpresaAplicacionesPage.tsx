...
export default function EmpresaAplicacionesPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-6 pb-6 pt-2 lg:px-8 lg:pb-8 lg:pt-3">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button onClick={() => setOpenCreate(true)} className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:bg-primary-hover">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {filtered.map((a) => (
                    <tr key={a.id} className="hover:bg-slate-50">
                      <td className="px-4 py-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-accent text-brand-primary"><ClipboardList className="h-5 w-5" /></span><div><Link to={`/psicosocial/empresas/${empresaId}/aplicaciones/${a.id}`} className="block font-black text-slate-950 hover:text-brand-primary hover:underline">{a.nombre}</Link><span className="text-xs text-slate-500">#{a.id}{a.created_at || a.creado_en ? ` · ${new Date(a.created_at || a.creado_en || "").toLocaleDateString()}` : ""}</span></div></div></td>
      ...
      {openCreate && (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/45 backdrop-blur-sm">
          <aside className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-2xl">
            <div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-brand-primary">Nueva aplicación</p><h2 className="text-2xl font-black text-slate-950">Crear batería psicosocial</h2><p className="mt-1 text-sm text-slate-500">La batería queda vinculada a esta empresa y crea las evaluaciones seleccionadas.</p></div><button onClick={() => setOpenCreate(false)} className="rounded-2xl border p-2 hover:bg-slate-50"><X className="h-5 w-5" /></button></div>
            <form onSubmit={crear} className="space-y-5">
              <label className="space-y-1 text-sm font-bold text-slate-700">Nombre de la batería *<input value={form.nombre} onChange={(e) => updateForm("nombre", e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-200 px-4 py-3 font-normal outline-none focus:border-input-focus focus:ring-4 focus:ring-input-focus/20" placeholder="Ej. Batería planta operativa mayo 2026" />{fieldErrors.nombre && <span className="block text-xs font-semibold text-red-600">{fieldErrors.nombre}</span>}</label>
              <AbrilDatePicker label="Fecha de aplicación" value={form.fecha_aplicacion} onChange={(value) => updateForm("fecha_aplicacion", value)} />
              <div className="rounded-2xl border border-slate-200 p-4"><div className="mb-3 flex items-center gap-2 font-black"><CalendarDays className="h-4 w-4 text-brand-primary" /> Instrumentos</div><div className="grid gap-3 sm:grid-cols-2">{([
                ["include_intra_a", "Intralaboral Forma A"],
                ["include_intra_b", "Intralaboral Forma B"],
                ["include_extra", "Extralaboral"],
                ["include_estres", "Estrés"],
              ] as const).map(([key, label]) => <label key={key} className="flex items-center gap-3 rounded-2xl border p-3 text-sm font-bold"><input type="checkbox" checked={form[key]} onChange={(e) => updateForm(key, e.target.checked)} className="h-4 w-4 accent-[hsl(var(--primary))]" />{label}</label>)}</div>{fieldErrors.instrumentos && <span className="mt-2 block text-xs font-semibold text-red-600">{fieldErrors.instrumentos}</span>}</div>
              <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={() => setOpenCreate(false)} className="rounded-2xl border px-5 py-3 font-bold">Cancelar</button><button disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-3 font-bold text-primary-foreground disabled:opacity-60">{saving && <Loader2 className="h-4 w-4 animate-spin" />} Crear batería</button></div>
            </form>
          </aside>
        </div>
      )}
