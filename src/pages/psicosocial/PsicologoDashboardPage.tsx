...
export default function PsicologoDashboardPage() {
  ...
  return <main className="space-y-6">
    ...
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black text-slate-950">Acciones rápidas</h2><div className="mt-4 grid gap-4 md:grid-cols-6"><Action icon={<Plus />} label="Nueva empresa" onClick={() => navigate('/psicosocial/empresas')} /><Action icon={<ClipboardCheck />} label="Nueva batería" onClick={() => navigate('/psicosocial/empresas')} /><Action icon={<Upload />} label="Carga masiva" onClick={() => navigate('/psicosocial/empresas')} /><Action icon={<Users />} label="Ver participantes" onClick={() => navigate('/psicosocial/empresas')} /><Action icon={<BarChart3 />} label="Resultados" onClick={() => navigate('/psicosocial/resultados')} /><Action icon={<FileText />} label="Informes" onClick={() => navigate('/psicosocial/reportes-oficiales')} /></div></section>
  </main>;
}
