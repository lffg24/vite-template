    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-brand-primary">Riesgo Psicosocial</p>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button onClick={onRegister} className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground shadow-sm hover:bg-primary-hover">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="rounded-2xl border border-slate-100 bg-gradient-to-r from-white to-slate-50 p-5">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-full bg-accent text-2xl font-black text-brand-primary">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-2 inline-flex rounded-full bg-accent px-3 py-1 text-sm font-bold text-brand-primary">{perfil.tipo_cargo?.toLowerCase().includes('oper') || perfil.tipo_cargo?.toLowerCase().includes('aux') ? 'Forma B' : 'Forma A'}</p>
            </div>
            <div>
          ...
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-primary" style={{ width: `${Math.max(0, Math.min(100, Number(perfil.completitud_perfil ?? 0)))}%` }} />
          </div>
        </div>
      </div>
    </div>
